...
public class JpaQueryLanguage {
    ...
    public void findConditionTest(){
        String jpql = "from Customer where custName like ? and custSource like ?";
        Query query = entityManager.createQuery(jpql);
        query.setParameter(1 , "%3");
        query.setParameter(2 , "深圳市%");
        printResult(query);
    }

    @Test
    public void updateConditionTest(){
        String jpql1 = "update Customer set custName = ? where id = ?";
        String jpql2 = "update Customer set custName =:a where id = :b";
        Query query = entityManager.createQuery(jpql2);
        query.setParameter("a" , "戴灵飞");
        query.setParameter("b" , 4L);
        int i = query.executeUpdate();
        System.out.println(i);
    }

    @Test
    public void insertConditionTest(){
    }
